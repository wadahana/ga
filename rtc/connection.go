...
func findBestCodec(sdp *sdp.SessionDescription, encService encoders.Service, h264Profile string) (*webrtc.RTPCodec, encoders.VideoCodec, error) {
	...
	for _, md := range sdp.MediaDescriptions {
		for _, format := range md.MediaName.Formats {
			...
			log.Debugf("format: %s, payloadType: %d\r\n", format, payloadType);
			if err != nil || payloadType == 0 {
				continue;
			}
...
func newRemoteScreenPeerConn(grabber display.Grabber, encService encoders.Service) *RemoteScreenPeerConn {

	var servers []webrtc.ICEServer;
	for _, info := range core.Setting.Rtc.IceServers {
		s := webrtc.ICEServer {
			URLs: []string{info.Url},
		}
		if len(info.Username) > 0 && len(info.Password) > 0 {
			s.Username = info.Username
			s.Credential = info.Password
...
func (p *RemoteScreenPeerConn) ProcessOffer(strOffer string) (string, error) {
	...
	peerConn.OnDataChannel(func(dataChannel *webrtc.DataChannel) {
		...
		dataChannel.OnOpen(func() {
			log.Debugf("data channel open")
		})
		dataChannel.OnClose(func() {
			log.Debugf("data channel close")
		})
    	// Handle data channel
    	dataChannel.OnMessage(func(msg webrtc.DataChannelMessage) {
    		if !msg.IsString {
    			p.streamer.onMessage(msg.Data);
    		} else {
    			log.Debugf("Message from DataChannel '%s': '%s'\n", dataChannel.Label(), string(msg.Data))
    		}
	...
	log.Debugf("Using codec %s (%d) %s", webrtcCodec.Name, webrtcCodec.PayloadType, webrtcCodec.SDPFmtpLine)
	...
	encoder, err := p.encService.NewEncoder(encCodec, sourceSize, p.grabber.Fps(), p.grabber.Bitrate())
	if err != nil {
		return "", err
	}
