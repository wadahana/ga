...
func newRTCStreamer(track *webrtc.Track, screen *display.Grabber, encoder *encoders.Encoder, size image.Point) videoStreamer {
	return &rtcStreamer{
		track:   track,
...
func (s *rtcStreamer) onMessage(msg []byte) {
	screen := *s.screen
	screen.SendEvent(msg);
}
