...
type encoderFactory = func(size image.Point, frameRate int, bitrate int) (Encoder, error)
...
func (*EncoderService) NewEncoder(codec VideoCodec, size image.Point, frameRate int, bitreate int) (Encoder, error) {
	factory, found := registeredEncoders[codec]
	if !found {
		return nil, fmt.Errorf("Codec not supported")
	}
	return factory(size, frameRate, bitreate)
}
